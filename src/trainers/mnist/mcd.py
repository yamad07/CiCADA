...
class MaximumClassifierDiscrepancyTrainer(object):
    ...
    def train(self, epoch):
        ...
        self.classifier_a_optim = optim.Adam(
            self.classifier_a.parameters(), lr=1e-4)
        self.classifier_b_optim = optim.Adam(
            self.classifier_b.parameters(), lr=1e-4)
        self.source_optim = optim.Adam(
            self.source_encoder.parameters(), lr=1e-4)
    ...
    def _validate(self, e):
        for i, (source_data, source_labels, _) in enumerate(
            ...
            source_data_features = self.tsne.fit_transform(
                self.source_encoder(source_data).detach().cpu().numpy())
            plt.figure()
