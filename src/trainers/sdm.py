...
class SourceDomainModelingTrainer(object):
    ...
    def __init__(self, source_encoder, source_generator, source_discriminator,
            classifier_a, classifier_b, data_loader, validate_data_loader, experiment):
        ...
        self.source_discriminator_optim = optim.Adam(self.source_discriminator.parameters(), lr=1e-4)
    ...
    def train(self, epoch):
        ...
        for e in range(epoch):
            for i, (source_data, source_labels, target_data) in enumerate(self.data_loader):
                ...
                classifier_loss, discrepancy, discriminator_loss, generator_loss = self._train_source_modeling(source_data, source_labels)
            ...
            print("Epoch: {} D(x): {} D(G(x)): {}, d(p1, p2): {} L(F(G(x))): {}".format(e, discriminator_loss, generator_loss, discrepancy, classifier_loss))
            self._validate(e)
    ...
    def _train_source_modeling(self, source_data, source_labels):
        ...
        source_fake_features = self.source_generator(torch.cat((z, source_one_hot), dim=1))

        true_preds = self.source_discriminator(torch.cat((source_features.detach(), source_one_hot), dim=1))
        fake_preds = self.source_discriminator(torch.cat((source_fake_features.detach(), source_one_hot), dim=1))
        ...
        fake_preds = self.source_discriminator(torch.cat((source_fake_features, source_one_hot), dim=1))
        generator_loss = self.source_generator_criterion(fake_preds, torch.zeros(batch_size).long().to(self.device))
