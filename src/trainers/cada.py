...
class ConditionalAdversarialTrainer:
    ...
    def __init__(self, experiment, source_generator, classifier, target_encoder, domain_discriminator,
            randomized_g, randomized_f, data_loader, validate_data_loader):
        ...
        self.discriminator_optim = optim.Adam(self.domain_discriminator.parameters(), lr=1e-3)
        self.target_encoder_optim = optim.Adam(self.target_encoder.parameters(), lr=1e-3)
        self.classifier_optim = optim.Adam(self.classifier.parameters(), lr=1e-3)
        self.source_generator_optim = optim.Adam(self.source_generator.parameters(), lr=1e-3)
    ...
    def train(self, epoch):
        ...
        for e in range(epoch):
            for i, (source_data, source_labels, target_data, _) in enumerate(self.data_loader):
                ...
                discriminator_loss = self._training_discriminator(source_labels, target_data)
                classifier_loss = self._training_classifier(source_labels)
                target_encoder_loss = self._training_target_encoder(target_data, source_labels)
    ...
    def _training_discriminator(self, source_labels, target_data):
        ...
        source_domain_preds = self.domain_discriminator(source_mul_features.detach())
        source_preds = self.classifier(source_features)

        target_features = self.target_encoder(target_data)
        target_mul_features = self._randomized_multilinear_map(target_features)
        target_domain_preds = self.domain_discriminator(target_mul_features.detach())

        labels = torch.cat((torch.zeros(batch_size).long().to(self.device), torch.ones(batch_size).long().to(self.device)), dim=0)
    ...
    def _training_target_encoder(self, target_data, source_labels):
        ...
        domain_labels = torch.cat((torch.zeros(batch_size), torch.ones(batch_size)), dim=0).long().to(self.device)

        adversarial_loss = - self.target_encoder_criterion(domain_preds, domain_labels)
    ...
    def _visualize_features(self, e):
        for i, (target_data, _) in enumerate(self.validate_data_loader):
            ...
            one_hot = torch.zeros(batch_size, 10).to(self.device)
            random_labels = torch.eye(batch_size, 10).long()
            one_hot = one_hot.scatter_(1, random_labels.to(self.device), 1).detach()
            z = torch.randn(batch_size, 100).to(self.device).detach()
            source_features = self.source_generator(torch.cat((z, one_hot), dim=1))
            target_features = self.target_encoder(target_data)
            features = torch.cat((source_features, target_features), dim=0)
            features_2dim = self.tsne.fit_transform(features.detach().cpu().numpy())
            plt.figure()
            plt.scatter(features_2dim[:1000, 0], features_2dim[:1000, 1], c='pink')
            plt.scatter(features_2dim[1000:, 0], features_2dim[1000:, 1], c='yellow')
